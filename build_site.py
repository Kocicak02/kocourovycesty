...
# TEXTOVA ZNACKA VE VSECH DETAIL.HTML
def update_detail_brand(path):
    s = path.read_text(encoding='utf-8', errors='ignore')
    original = s

    # Pokud je v brand odkazu logo, nahrad ho textem.
    s = re.sub(
        r'(<a\\b[^>]*class=["\\\'][^"\\\']*\\bbrand\\b[^"\\\']*["\\\'][^>]*>).*?(</a>)',
        r'\\1Kocourovy cesty\\2',
        s,
        count=1,
        flags=re.S | re.I
    )

    # Odstranit CSS pro obrazkove logo.
    s = re.sub(r'\\.brand\\s+img\\s*\\{[^}]*\\}', '', s, flags=re.I)

    # Pokud .brand existuje, sjednotit jej na puvodni textovy styl.
    s = re.sub(
        r'\\.brand\\s*\\{[^}]*\\}',
        '.brand{font-size:18px;font-weight:700;letter-spacing:.06em;text-transform:uppercase}',
        s,
        count=1,
        flags=re.I
    )

    # Kdyz .brand CSS chybi, dopln ho.
    if '.brand{' not in s.replace(' ', ''):
        s = s.replace(
            '</style>',
            '.brand{font-size:18px;font-weight:700;letter-spacing:.06em;text-transform:uppercase}</style>',
            1
        )

    if s != original:
        path.write_text(s, encoding='utf-8')
        print('Textova znacka:', path)

for detail in Path('.').glob('*/detail.html'):
    update_detail_brand(detail)
