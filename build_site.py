...
(()=>{
  ...
  function imageUrlFromAnchor(a,img){
  ...
  function looksLikePhotoUrl(url){
    ...
    try{
      ...
      return /\.(jpe?g|png|webp|gif|avif|heic|heif)$/i.test(p);
    }catch(e){
  ...
  function photoItems(){
    ...
    // Schvalne bez zavislosti na ID galerii.
    // Bere vsechny odkazy s obrazkem, ktere skutecne vedou na fotku.
    document.querySelectorAll('a[href] img').forEach(img=>{
      if(img.closest('#photoLightbox'))return;
      const a=img.closest('a[href]');
      if(!a)return;

      const url=imageUrlFromAnchor(a,img);
      if(!looksLikePhotoUrl(url))return;
  ...
  function clickedGalleryImage(target){
    ...
    const url=imageUrlFromAnchor(a,img);
    if(!looksLikePhotoUrl(url))return null;

    // Nechceme chytat obrazky v hlavicce / navigaci, pokud by tam nekdy byly.
    if(img.closest('header,nav'))return null;
